...
const Conform = ({ data, close }) => {
  return (
    <section className="fixed inset-0 z-15 flex items-center justify-center min-h-screen w-full bg-black/10 backdrop-blur-sm overflow-hidden">
      <div className="w-sm mx-auto bg-white rounded-xl p-5 shadow-2xl">
        <div className="mx-auto pt-12 pb-8 px-5 text-center">
          <div className="inline-flex items-center justify-center w-12 h-12 mb-5 bg-gray-200 rounded-full">
            <svg
        ...
        <div className="flex justify-center items-center bg-gray-50 rounded-md gap-5 p-5">
          <button
            onClick={() => {
              data.Delete_Speaker(data.data.section, data.data.id);
              close({ isOpen: false });
            }}
          ...
          <button
            ...
            className="inline-block w-full sm:w-auto py-3 px-5 mb-2 mr-4 text-center font-semibold leading-6 text-gray-500 bg-gray-200 hover:bg-gray-300 rounded-lg transition duration-200"
          >
            Cancel
          </button>
        </div>
