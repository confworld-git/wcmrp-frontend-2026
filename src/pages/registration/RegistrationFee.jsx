...
// ─── Pricing Calculator ───────────────────────────────────────────────────────
// Membership discount : 5% off base
// Coupon discount     : 5% off base
// Both together       : 10% off base
// Membership fee      : $20 (non-student) or $15 (student) added back
// Bank tax            : 5.8% on the amount after discounts + membership fee
const calculatePricing = ({ baseAmount, participantCategory, hasMembership, hasCoupon }) => {
  ...
  if (hasMembership) {
    membershipDiscount = parseFloat((base * 0.05).toFixed(2));
    membershipFee = membershipFeeAmount;
  }
  if (hasCoupon) {
    couponDiscount = parseFloat((base * 0.05).toFixed(2));
  }

  const totalDiscount = parseFloat((membershipDiscount + couponDiscount).toFixed(2));
  const afterDiscount = parseFloat((base - totalDiscount + membershipFee).toFixed(2));
  const bankTax = parseFloat((afterDiscount * 0.060).toFixed(2));
  const total = parseFloat((afterDiscount + bankTax).toFixed(2));
  ...
  return {
    ...
    totalDiscount,
    membershipFee,
    finalAmount: afterDiscount,   // subtotal before bank tax
    bankTax,
    total,                        // grand total charged to Razorpay
  };
};
...
const CheckoutPanel = ({
  ...
  return (
    <div className="md:w-[420px] shrink-0 space-y-4">
      ...
      {/* Coupon */}
      <div className="rounded-2xl border border-indigo-200 bg-indigo-50 p-5 shadow-sm">
        <h3 className="text-[15px] font-bold text-indigo-600 mb-1">
...
const RegistrationFee = () => {
  ...
  // Derived pricing — recalculates on every relevant state change
  ...
  const HandleRegistration = async (e) => {
    ...
    if (!RegistrationData.Terms_and_Conditions) {
      toaster.warning("Please accept the Terms and Conditions.");
      return;
    }
    if (!RegistrationData.presentation_Category) {
      toaster.warning("Please select a Presentation Category.");
      return;
    }
    if (!RegistrationData.presentation_Type) {
      toaster.warning("Please select a Presentation Type.");
      return;
    }
    if (!RegistrationData.participant_category) {
      toaster.warning("Please select a Participant Category.");
      return;
    }
    if (!RegistrationData.Title) {
      toaster.warning("Please select a Title.");
      return;
    }

    // Build the discountApplied value: 10 if both, 5 if one, 0 if neither
    const discountApplied =
      membership && couponDiscount > 0 ? 10
      : membership || couponDiscount > 0 ? 5
      : 0;
    ...
    try {
      ...
      const options = {
        ...
        description: "WCMRP-2026 Conference Registration",
        image: logoImage,
        ...
        handler: async (response) => {
          try {
            ...
            navigate("/Payment_Details_Success", {
              state: {
        ...
        prefill: {
          ...
          contact: RegistrationData.number,
        },
        theme: { color: "#4f46e5" },
        modal: {
          ondismiss: async () => {
      ...
      toaster.danger(error.response?.data?.message || "Payment processing failed.");
      console.error(error);
    }
  };
  ...
  const participationFees1 = [
    {
      ...
      categories: [
        { category: "Academicians / Delegates / Research Scholars / PhD Student", earlyBird: 399, final: 449, onsite: 549 },
        { category: "Academicians / Delegates / Research Scholars / PhD Student with Scopus Publication", earlyBird: 849, final: 899, onsite: 999 },
        { category: "Students", earlyBird: 349, final: 399, onsite: 499 },
        { category: "Students with Scopus Publication", earlyBird: 749, final: 799, onsite: 899 },
        { category: "In-Person Attendance / Listener (Non-Presenter)", earlyBird: 199, final: 249, onsite: 349 },
      ],
    },
  ];
  ...
  const participationFees2 = [
    {
      ...
      categories: [
        { category: "Academicians / Delegates / Research Scholars / PhD Student", earlyBird: 299, final: 349 },
        { category: "Academicians / Delegates / Research Scholars / PhD Student with Scopus Publication", earlyBird: 799, final: 849 },
  ...
  const discountAndPaperPresentation = [
    {
      ...
      details: [
        "You qualify for a discounted registration fee if you are a group of 5 members or more individuals or if you are a co-author of a paper presentation.",
    ...
    {
      title: "If you are presenting more than one paper",
      details: [
        "An author may submit and present a maximum of three papers at the conference.",
        "If you are presenting more than one paper at the conference, full payment is required for the first paper.",
        "If other papers are oral or poster presentations, an additional fee of 150 USD will be charged for each paper.",
        "If any of the papers requires Scopus publication, then you have to pay the publication fee for each paper.",
        "If you have more than 3 papers, the additional paper can be presented by a co-author on full registration.",
    ...
    <section>
      ...
      <div className="flex flex-col justify-center items-center text-center my-5 space-y-4">
        ...
        <p className="w-11/12 font-medium">
          Welcome to the WCMRP-2026 registration page. Secure your spot at this premier conference to connect with global experts, present your research and advance your career. Follow the steps below to complete your registration.
        </p>
      </div>
      ...
      <div className="bg-indigo-50 flex flex-col justify-center items-center p-5 gap-5 md:gap-10">
        {discountAndPaperPresentation.map((item, index) => (
          <div key={index} className="bg-cyan-700 text-white rounded-xl p-5 md:px-15 md:py-10 md:w-9/12 space-y-4 overflow-hidden">
            ...
            <ul className="font-medium leading-6 space-y-4">
      ...
      <div className="flex flex-col justify-center items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:p-5 w-full">
          {participationFees1.map((fee, index) => (
            <table key={index} className="table-auto w-full border-collapse rounded-b-md shadow-xl">
              ...
              <thead>
                <tr className="bg-emerald-500 text-md text-white text-left">
                ...
                {fee.categories.map((item, i) => (
                  <tr key={i} className={`${i % 2 === 0 ? "bg-gray-100" : "bg-white"} text-sm font-[450]`}>
                    <td className="border-r border-gray-200 p-4">
                      {item.category}
                      {["Students", "Students with Scopus Publication"].includes(item.category) && <span className="text-red-600">*</span>}
                    </td>
                    <td className="border-r border-gray-200 p-4">
                    ...
                    <td className="p-4">
                      <label>
                        <input type="radio" name="price" onChange={() => handleBaseSelect(item.onsite, fee.title, item.category)} value={item.onsite} className="mr-2" />
                        {item.onsite}
                      </label>
                    </td>
                  </tr>
        ...
        <div className="w-full px-4 text-left">
          <p className="text-red-600 text-sm font-medium">
            *Indicates - UG students only (You have to submit a soft copy of your university/college identity card as a proof)
          </p>
        </div>
      ...
      <div className="flex flex-col justify-center items-center mt-4 p-5 md:py-10">
        ...
        <p className="text-md font-medium mt-2">
          "2<span className="text-sm align-super">nd</span> World Conference on Multidisciplinary Research and Practices (WCMRP-2026)"
        </p>
        ...
        >
          ...
          <section className="w-full space-y-4 md:columns-2 gap-5">
            ...
            {/* Controlled — needed so membership fee updates correctly */}
            <div>
              <select
          ...
          <div className="mt-6 md:mt-0 w-full md:w-auto">
            ...
            <div className="mt-4 space-y-3">
              ...
              <button
                type="submit"
                className="w-full py-3.5 bg-gradient-to-r from-teal-600 to-teal-500 text-white font-bold text-base rounded-2xl hover:from-teal-500 hover:to-teal-600 transition-all shadow-md hover:shadow-lg"
              >
